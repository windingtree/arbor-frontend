 ...
 * API
 */
const apiSendToggleActiveState = (data, gasPrice) => {
...
const apiGetTxStatus = transactionHash => {
    return new Promise((resolve, reject) => {
        const web3 = getWeb3();
 ...
 * Sagas
 */
function* startTransactionStatusSaga ({ payload }) {
    try {
        yield call(apiGetTxStatus, payload.tx);
        yield put(transactionStatusSuccess());
        const organization = yield select(selectItem);
        yield put(fetchOrganizationInfoWithRefresh({ id: organization.orgid }));
        yield put(fetchProfileOrganizations({ owner: organization.owner }));
...
function* startSaveActiveStatusSaga () {
    try {
        ...
        yield put(getActiveStatusTransactionStatus({ tx }));
    } catch (error) {
