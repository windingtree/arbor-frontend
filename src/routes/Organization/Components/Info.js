...
import { TwitterIcon, FacebookIcon, LinkedInIcon, GitHubIcon, TrustLevelIcon, EntityTrustLevelIcon, TelegramSocialIcon, MediumSocialIcon, InstagramSocialIcon, DetailsIllustration, MaximizeIcon, MinimizeIcon } from '../../../assets/SvgComponents';
...
function Info(props) {
  ...
  const description = _.get(organization, `jsonContent.organizationalUnit.description`, false);
  const longDescription = _.get(organization, `jsonContent.organizationalUnit.longDescription`, false);
  ...
  _.each(possibleSocial, (short, name) => {
    if (contacts[name]) socials.push({ network: name, link: contacts[name], verified: !!organization[`isSocial${short}Proved`]})
  });
            ...
            socials.map((social, index) => {
              return (
                <a key={index.toString()} href={social.link} className={classes.socialLink}>
                  ...
                  {social.verified && <TrustLevelIcon className={[classes.iconTrustLevel, classes.iconVerify].join(' ')}/>}
                </a>
              )
            })
          }
